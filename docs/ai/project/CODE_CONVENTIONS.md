...
### React / Next.js
- Use App Router (`src/app`) with Server Components by default; opt into Client Components only when needed (`"use client"`).
- Place route handlers in `app/**/route.ts` and server actions in appropriate server files.
- Keep components pure; colocate styles and tests near components.
- Avoid deep prop drilling; use context sparingly and memoization (`useMemo`, `useCallback`) when profiling indicates.
...
src/
  ...
  components/         # Reusable UI components (Client when needed)
  lib/                # Utilities (e.g., supabase clients, helpers)
